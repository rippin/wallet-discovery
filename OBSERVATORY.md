...
- RPC credits are split 50% tracking, 30% discovery, 20% revisits, with per-day and per-calendar-month ceilings. An exhausted bucket pauses until its next allowance. Local accounting cannot see other applications using the same key; dedicate a key/account allowance and inspect provider usage. Helius local budget limits and HTTP/RPC 429 responses fall back to https://solana-rpc.publicnode.com. PublicNode is capped at one request per second and 10,000 requests per UTC day, tracked separately from primary credits. A 429 pauses primary requests for one hour before probing again; fallback errors pause public requests for five minutes. Cooldowns and public request counts persist across restarts. Authentication errors do not trigger fallback. Provider switches appear in System events. Public service availability and history are not guaranteed.
...
## Purchase-size admission

`OBS_MIN_PURCHASE_USD=100` requires each new wallet's discovery purchase to be worth at least $100 equivalent, in addition to the market-cap rule. Set 0 to disable. SOL and arbitrary quote tokens use a separate, batched DEX Screener quote-price cache; missing prices remain pending. Each purchase value is frozen when first valued using a quote observation at most ten minutes old. This is an estimate at observation time, not a reconstructed execution-time USD cost, and routed quote amounts are instruction-local. A previously rejected small purchase does not become eligible merely because its quote currency later appreciates. Existing/manual wallets retain all trade observations. Filtering saves follow-up wallet scans, not the initial transaction read. Health shows below-threshold and unknown-valuation counts.
